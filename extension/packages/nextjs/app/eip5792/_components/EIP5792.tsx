...
export const EIP5792 = () => {
  ...
  const { data: YourContract } = useDeployedContractInfo("Example");
  const { writeContractAsync: writeYourContractAsync, isPending: isWriteYourContractPending } =
    useScaffoldWriteContract("Example");
  ...
  const handleBatchTransaction = async () => {
    try {
      ...
      const paymasterURL =
        chainId === sepolia.id
          ? process.env.NEXT_PUBLIC_BASE_SEPOLIA_PAYMASTER_URL
          : process.env.NEXT_PUBLIC_SEPOLIA_PAYMASTER_URL || `${baseUrl}/api/paymaster`;
